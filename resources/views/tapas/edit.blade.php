...
<div class="mt-3 mx-auto p-2 w-4/5" id="formCorto">
  <form name="f" action="{{route('tapas.update',$tapa)}}" method="POST">
      @csrf
      @method('PUT')
      <div class="col">
          <label for="nombre" class="col-form-label">Nombre</label>
          <input type="text" class="form-control" name="nombre" placeholder="Nombre" id="nombre" value={{$tapa->nombre}} required>
      </div>
      <div class="col">
        <label for="precio" class="col-form-label">Precio</label>
        <input type="number" class="form-control" name="precio" placeholder="Precio" id="nombre" value={{$tapa->precio}} step="any" required>
     </div>
      <div class="row mt-4">
          @php
              ...
              <select name="tipotapa_id" class="form-select form-select-sm" aria-label=".form-select-sm example">
                  @foreach ($tipos as $item)
                      <option value="{{$item->id}}"
                          @if(($item->id)==($tapa->tipotapa_id)) selected @endif>
                      {{$item->nombre}}</option>
                  @endforeach
              </select>
          </div>
